...
# This file was *autogenerated* from the file /Users/Eliver/GitHub/ASN.1/encoding/decodeAndSolve.sage
from sage.all_cmdline import *   # import sage library

_sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_4 = Integer(4); _sage_const_30 = Integer(30); _sage_const_25 = Integer(25)
from brial        import *
...
def getFileHandler(msg, mode):
    ...
    if ( fName != fName.replace(' ', '') ):
        print "\nFile name error: '{0}'\n".format(fName)
        fh = _sage_const_0      # No file handler
        RC = _sage_const_1 
    else:
        RC = _sage_const_0 
        try:
    ...
    for line in fi:         # Read file
        ...
        for i in range( len(line) ):
            ...
            if ( fixing and ( ch not in nums ) ):           # x(nn) not EOL
                ...
                maxNumVar = ''
                continue
            elif ( fixing and ( i == ( len(line) - _sage_const_1  ) ) ): # x(nn) in EOL
                fixing = False
                aux += ch + ')'
                if ( maxNVInt < int( maxNumVar ) ): maxNVInt = int( maxNumVar )
                maxNumVar = ''
...
def getInteger(msg):
    ...
    try:			# Is a number?
        ...
        intN  = -_sage_const_1 
        RC    =  _sage_const_1 
    if ( ( not error ) and ( intNumber == _sage_const_0  ) ):
        print "\nNumber error: '{0}'\n".format(intNumber)
        intN = -_sage_const_1 
        RC   =  _sage_const_2 
    return RC, intN
...
def genPolynomials(n, m, vars, F, d, K):
    binSize = ((n * (n + _sage_const_1 ) * d) / _sage_const_2 )
    p = _sage_const_0 
    PolySet = []
    pol = []
    for i in range(m):
        pol.append(_sage_const_0 )
    ...
    for i in range(m):
        ...
        for j in range(n):
            for k in xrange(j, n):
                ...
                if(d > _sage_const_1 ):
                    poly += F.fetch_int(int(coef, _sage_const_2 )) * (vars[j] * vars[k])
                else:
                    poly += int(coef,_sage_const_2 ) * (vars[j] * vars[k])
                #PolySet.append(poly)
                PolySet[i] = poly
    return PolySet, p
...
def writePolys(PolySet, m, n, q):
	...
	file.write(str(PolySet[m - _sage_const_1 ]))
	return filename
...
def iniVarsAssig(noVars):
    ...
    return [ -_sage_const_1  for i in range(noVars) ]
...
def randVect(size):
    ...
    vect = []
    for i in range(size):
        vect.append( random.randint(_sage_const_0 ,_sage_const_1 ) )
    return vect
...
def endOfPoly(poly):
    ''' Analyse "poly" and returns a code: if it is empty (1), or has only one variable
        ...
        Otherwise returns (0) '''
    if ( poly == _sage_const_0  ): 			# Polynomial empty (0)
        RC = _sage_const_1 
    elif ( (not poly.has_constant_part()) and
           (poly.nvariables() == _sage_const_1  )):	# Number variables equal 1
        RC = _sage_const_2 
    elif ( poly.has_constant_part() and (poly.nvariables() == _sage_const_1 ) ): 	# One variable + 1
        RC = _sage_const_3 
    elif ( poly.has_constant_part() and (poly.nvariables() == _sage_const_0 ) ):    # Just 1
        RC = _sage_const_4 
    else:
        RC = _sage_const_0 
    return RC
...
def writeEvaluationFile(polynomials, variables):
	...
	varVal = "("
	for i in range(numVars - _sage_const_1 ):
		varVal += str(variables[i]) + ","
	varVal += str(variables[numVars - _sage_const_1 ]) + ")"
	for i in range(len(polynomials) - _sage_const_1 ):
		file.write("evaluation.append(polynomials[" + str(i) + "]" + varVal + ")\n")
	file.write("evaluation.append(polynomials[" + str(len(polynomials) - _sage_const_1 ) + "]" + varVal + ")")
	file.close()
...
def decodePolynomials(polynomials, n, m, vars, F, d, K):
        binSize = ((n * (n + _sage_const_1 )) / _sage_const_2 ) * d
        PolySet = []
        pol = []
        for i in range(m):
            pol.append(_sage_const_0 )
        ...
        for i in range(m):
            ...
            for j in range(n):
                for k in xrange(j, n):
                    ...
                    coef = polynomial[index:index + d]
                    z += _sage_const_1 
                    coefficient = _sage_const_0 
                    for l in range(d-_sage_const_1 ):
                        if coef[l] == "1":
                            coefficient += x**((d - _sage_const_1 )-l)
                    if coef[d-_sage_const_1 ] == "1":
...
k = GF(p**d, GF(p)['X'].irreducible_element(d), names=('x',)); (x,) = k._first_ngens(1)
...
maxNV    += _sage_const_1 				    			# Not an index but a quantity
...
for i in range( len(gb) ):
    ...
    while ( True ):                             # If there are monomials with deg>=1 do:
        ...
        if ( len(listMDge1) > _sage_const_0  ):              # If there are monomials left:
            var = getFstVarMon( listMDge1[_sage_const_0 ] )	# Get first variable of monomial with deg>1
        else:
            break
        if (Debug): print "Variable to assign value: {0}.".format(var),
        status = endOfPoly(poly)
        if   ( status == _sage_const_0  ):   # Several monomials in poly, still pending
            addVarAssig(listVars, var.index(), _sage_const_0 )
            if (Debug): print "Assigned value:",_sage_const_0 
            poly = subsVarsAssig(poly, listVars)
            continue
        elif ( status == _sage_const_1  ):   # All terms in "poly" evaluated to cero
            break               # so, continue with next polynomial
        elif ( status == _sage_const_2  ):   # Just one variable left, set it as zero and next poly
            addVarAssig(listVars, var.index(), _sage_const_0 )
            if (Debug): print "Assigned value:",_sage_const_0 
            break
        elif ( status == _sage_const_3  ):   # Just one variable left plus a constant,
            addVarAssig(listVars, var.index(), _sage_const_1 )   # so, set it as one and next poly
            if (Debug): print "Assigned value:",_sage_const_1 
            break
